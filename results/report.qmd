...
After increasing the number of top predictive proteins from 10 to 16, we observed a significant improvement in the model's performance metrics. Accuracy of the model increased from 0.64 to 0.81, this indicates that the model became better at correctly classifying instances of both ASD and typically developing controls as more predictive proteins were included in the analysis. The area under the curve (AUC) also increased from 0.795 to 0.875, indicating a high classification accuracy. Sensitivity had a dramatic increase from 0.462 to 0.812. This means that the updated model is now much better at identifying individuals with Autism Spectrum Disorder (ASD), reducing the number of false negatives. While sensitivity improved, specificity slightly decreased from 0.833 to 0.8, indicating a marginally higher false positive rate. This trade-off is typical when increasing sensitivity, as the model becomes more inclusive in positive predictions.
